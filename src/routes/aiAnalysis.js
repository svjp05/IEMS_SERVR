...
router.get('/patterns', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: { message: '服务器处理分析请求时出错', details: error.message } });
  }
});
...
router.get('/anomalies', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: { message: '服务器处理异常检测请求时出错', details: error.message } });
  }
});
...
router.get('/prediction', async (req, res) => {
  try {
    ...
    if (duration <= 0) {
      return res.status(400).json({ error: { message: '预测时长必须大于0' } });
    }
    ...
    if (!trainingData || trainingData.length === 0) {
      return res.status(404).json({ error: { message: '在指定训练时间范围内未找到数据' } });
    }
    ...
    res.status(500).json({ error: { message: '服务器处理预测请求时出错', details: error.message } });
  }
});
...
router.post('/query', async (req, res) => {
  try {
    ...
    if (!prompt || prompt.trim() === '') {
      return res.status(400).json({ error: { message: '查询内容不能为空' } });
    }
    ...
    res.status(500).json({ error: { message: '服务器处理AI问答请求时出错', details: error.message } });
  }
});
